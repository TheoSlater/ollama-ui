...
/**
 * Service for managing Ollama models (list, pull, remove, show)
 */
export class OllamaModelService {
  ...
  private parseModelList(output: string): OllamaModel[] {
    console.log(`[OllamaModelService] Raw ollama list output:\n${output}`);

    const lines = output.trim().split("\n");
    console.log(`[OllamaModelService] Split into ${lines.length} lines`);

    // Skip header line
    if (lines.length <= 1) {
      console.warn(
        `[OllamaModelService] No data lines found (only ${lines.length} lines total)`
      );
      return [];
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (!line) {
        console.log(`[OllamaModelService] Skipping empty line ${i}`);
        continue;
      }
      ...
      try {
        ...
        if (model) {
          models.push(model);
          console.log(
            `[OllamaModelService] Successfully parsed model: ${model.name}`
          );
        }
      } catch (error) {
        console.warn(
          `[OllamaModelService] Failed to parse model line: ${line}`,
          error
        );
      }
    }

    console.log(`[OllamaModelService] Parsed ${models.length} models total`);
    return models;
  }
  ...
  private parseModelLine(line: string): OllamaModel | null {
    ...
    console.log(`[OllamaModelService] Parsing model line: "${line}"`);

    const parts = line.trim().split(/\s+/);
    if (parts.length < 4) {
      console.warn(
        `[OllamaModelService] Insufficient parts in line: ${parts.length}`
      );
      return null;
    }
    ...
    console.log(
      `[OllamaModelService] Parsed: name=${name}, digest=${digest}, size=${sizeStr}, modified=${modified_at}`
    );
